...
def main(args):
    ...
    for file in os.listdir(args.directory):
        if file.endswith(".csv"):
            match = re.search(r'per(\d+)', file)
            if match:
                per = int(match.group(1))
                time = int ( re.search(r'(\d+)s', file).group(1) )
                files.append([file, per, time])

    graph = []
    for file in files:
        nevent, nhit =  matching(os.path.join(args.directory, file[0]))
        graph.append([file[1], file[2], nevent, nhit])

    df_graph = pd.DataFrame(graph, columns=["per", "time", "nevent", "nhit"])
    ...
    df_graph["frequency"] = df_graph["per"].map(per_to_frequency)
    df_graph["injection"] = df_graph["frequency"] * df_graph["time"]
    df_graph["efficiency"] = df_graph["nhit"]/df_graph["injection"]*100
    
    df_graph = df_graph.sort_values('frequency').sort_values('time')
    
    df_by_group_label = df_graph.reset_index()
    df_pivot = df_by_group_label.pivot(index='frequency',columns='time',values='efficiency')
    ...
    for ievt in dff['readout'].unique():
        ...
        for indc in df_col.index:
            for indr in df_row.index:
                ...
                if col != inj_pixel[1]: continue
                if row != inj_pixel[0]: continue

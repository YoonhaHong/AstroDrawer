...
def create_qc_map(csv_file, outdir):
    try:
        ...
        for _, row in df.iterrows():
            ...
            asc_count = 0
            for v in tot_values:
                if v < 0: continue  # 0이 아닌 값에 대해서만 체크
                elif v > prev_val:
                    asc_count += 1
                    prev_val = v
            ...
            if all(v == 0 for v in tot_values):  # 모든 값이 0 (비어있는 경우)
                qc_map[r, c] = 'black'
            elif asc_count > 6: # 7 points are ascending
                qc_map[r, c] = 'green'
            else:
                qc_map[r, c] = 'orange'
                text_annotations[r, c] = str(asc_count)
        ...
        plt.tight_layout()
        plt.savefig(f"{outdir}/QCMap3_TU{args.tot_thr}_{dir_name}.pdf", bbox_inches='tight')

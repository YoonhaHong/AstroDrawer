        ...
        for col in range(COL_MAX):
            ...
            for file in pixel_files:
                ...
                if match:
                    ...
                    tot_means[vinj] = fit_mean  # ToT mean 저장
...
def create_qc_map(csv_file, outdir):
    try:
        ...
        # ToT mean 컬럼만 선택
        tot_columns = [col for col in df.columns if 'ToT_mean_' in col]
        # Vinj 값 순서대로 정렬
        ...
        for _, row in df.iterrows():
            ...
            elif all(v < args.tot_thr for v in tot_values if v > 0):  # 모든 값이 ToT_THR 미만
                qc_map[r, c] = 'red'
                text_annotations[r, c] = ''
            elif any(v < args.tot_thr for v in tot_values if v > 0):  # 일부 값이 ToT_THR 미만
                qc_map[r, c] = 'orange'
                low_count = sum(1 for v in tot_values if v > 0 and v < args.tot_thr)
        ...
        plt.ylim(ROW_MAX-0.5, -0.5)  # y축 반전
        
        # 범례 추가
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor='black', label='Empty'),
            plt.Rectangle((0,0),1,1, facecolor='red', label=f'All ToT < {args.tot_thr}'),
            plt.Rectangle((0,0),1,1, facecolor='orange', label=f'Some ToT < {args.tot_thr}'),
            plt.Rectangle((0,0),1,1, facecolor='green', label='Good (Increasing)'),
            plt.Rectangle((0,0),1,1, facecolor='white', label=f'Else')
        ]
...
def create_injection_map(directory, outdir):
    # CSV 파일 경로 설정
    dir_name = os.path.basename(os.path.normpath(directory))
    csv_path = f"{outdir}/InjectionMap_Results_{dir_name}.csv"
    ...
    if os.path.exists(csv_path):
        ...
        return create_qc_map(csv_path, outdir)
    
    print("새로운 CSV 파일을 생성합니다...")
    if create_new_map(directory, outdir):
        return create_qc_map(csv_path, outdir)
    return False
...
if __name__ == "__main__":
    ...
    create_injection_map(args.directory, args.outdir)